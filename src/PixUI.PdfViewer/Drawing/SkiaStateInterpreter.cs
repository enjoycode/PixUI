...
{
    internal static IPaint Brush(this SkiaGraphicsState state)
    {
    ...
    internal static IPaint Pen(this SkiaGraphicsState state)
    {
    ...
    private static void SetDashState(GraphicsState state, IPaint paint)
    {
        paint.PathEffect = state.IsDashedStroke() ? CreatePathEffect(state.DashArray, state.DashPhase) : null;
    }

    private static IPathEffect CreatePathEffect(IReadOnlyList<double> dashes, double phase) =>
