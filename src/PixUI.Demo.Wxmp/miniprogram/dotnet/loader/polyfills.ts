...
export async function detect_features_and_polyfill(module: DotnetModuleInternal): Promise<void> {
  ...
  const scriptUrlQuery = /*! webpackIgnore: true *//*import.meta.url*/ "/blazor/_framework/";
  ...
  loaderHelpers.locateFile = (path) => {
    ...
    //     return new globalThis.URL(path, loaderHelpers.scriptDirectory).toString();
    // }
  ...
  if (ENVIRONMENT_IS_NODE) {
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore:
    INTERNAL.require = await import(/*! webpackIgnore: true */"module").then(mod => mod.createRequire(/*! webpackIgnore: true *//*import.meta.url*/"/blazor/_framework/"));
  } else {
...
export async function fetch_like(url: string, init?: RequestInit): Promise<Response> {
  try {
    ...
    //         url,
    //         headers: { length: 0, get: () => null },
    //         arrayBuffer: () => { return new Uint8Array(read(url, "binary")); },
    //         json: () => { return JSON.parse(read(url, "utf8")); },
    //         text: () => read(url, "utf8")
    //     };
    // }
    ...
    return new Promise<Response>((resolve, reject) => {
      wx.request({
        url: url,
        method: init?.method,
        responseType: init?.responseType ?? "arraybuffer",
        dataType: init?.responseType == "text" ? "json" : "其他",
        success: buf => {
          resolve(<Response><any>{
            ok: true,
            headers: { length: 0, get: (n) => buf.header[n] },
            url,
