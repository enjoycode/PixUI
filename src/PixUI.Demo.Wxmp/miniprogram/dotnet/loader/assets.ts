...
import { WasmEnableThreads}  from '../consts'

import { PThreadPtrNull, AssetEntryInternal, PThreadWorker, PromiseAndController } from "../types/internal";
import { BootModule, AssetBehaviors, AssetEntry, LoadingResource, ResourceList, SingleAssetBehaviors as SingleAssetBehaviors, WebAssemblyBootResourceType } from "../types/index1";
...
export async function streamingCompileWasm () {
    try {
        ...
        //let compiledModule: WXWebAssembly.Module;
        // if (typeof WXWebAssembly.compileStreaming === "function" && contentType === "application/wasm") {
        //     compiledModule = await WXWebAssembly.compileStreaming(response);
        // } else {
        ...
        //         compiledModule = await Promise.resolve(new WXWebAssembly.Module(arrayBuffer));
        //     } else {
        //         compiledModule = await WXWebAssembly.compile(arrayBuffer!);
        //     }
        // }
        //Rick
        let compiledModule = "/dotnet/dotnet.native.wasm.br";
