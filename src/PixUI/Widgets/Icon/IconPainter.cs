using System;
using System.Threading.Tasks;

namespace PixUI;

public sealed class IconPainter : IDisposable
{
    public IconPainter(Action onFontLoaded)
    {
        _onFontLoaded = onFontLoaded;
    }

    private readonly Action _onFontLoaded;
    private IFont? _cachedFont;
    private ushort _cachedGlyphId;
    private bool _loading;

    public void Paint(ICanvas canvas, float size, in Color color, in IconData data,
        float offsetX = 0, float offsetY = 0)
    {
        if (_cachedFont == null)
        {
            var typeface = FontCollection.TryMatchFamilyFromAsset(data.Asset.FontFamily);
            if (typeface == null)
            {
                if (!_loading)
                {
                    _loading = true;
                    FontCollection.FontChanged += _OnFontChanged;
                    StartLoadFontFromAsset(data.Asset.AssemblyName, data.Asset.AssetPath, data.Asset.FontFamily);
                }

                return;
            }

            _cachedFont = typeface.MakeFont(size);
            _cachedGlyphId = _cachedFont.GetGlyphId(data.CodePoint);
        }

        var paint = PixUI.Paint.Shared(color);
        canvas.DrawGlyph(_cachedGlyphId, offsetX, size + offsetY, 0, 0, _cachedFont!, paint);
    }

    private static bool StartLoadFontFromAsset(string asmName, string assetPath, string familyName)
    {
        if (FontCollection.HasLoading(familyName))
            return false;

        Task.Run(() =>
        {
            var stream = AssetLoader.LoadAsStream(asmName, assetPath);
            if (stream == null) return;

            UIApplication.Current.BeginInvoke(() =>
            {
                FontCollection.RegisterTypeface(stream, familyName, true);
                stream.Dispose();
            });
        });

        return true;
    }

    private void _OnFontChanged()
    {
        FontCollection.FontChanged -= _OnFontChanged;
        _onFontLoaded();
    }

    public void Reset()
    {
        _cachedFont?.Dispose();
        _cachedFont = null;
        _cachedGlyphId = 0;
        _loading = false;
    }

    public void Dispose()
    {
        _cachedFont?.Dispose();
    }
}