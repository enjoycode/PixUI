...
public sealed unsafe class SKPositionedRunBuffer : SKRunBuffer
{
    ...
    public Span<Point> GetPositionSpan() =>
        new Span<Point>(internalBuffer.pos, internalBuffer.pos == null ? 0 : Size);

    public void SetPositions(ReadOnlySpan<Point> positions) =>
        positions.CopyTo(GetPositionSpan());
}
