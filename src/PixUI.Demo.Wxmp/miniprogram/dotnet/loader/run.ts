...
import { BuildConfiguration } from "../consts";

import { MonoConfig, DotnetHostBuilder, DotnetModuleConfig, RuntimeAPI, LoadBootResourceCallback } from "../types/index1";
...
function importModules () {
    ...
    if (typeof jsModuleRuntimeAsset.moduleExports === "object") {
        ...
        //Rick jsModuleRuntimePromise = import(/*! webpackIgnore: true */jsModuleRuntimeAsset.resolvedUrl!);
        jsModuleRuntimePromise = new Promise<RuntimeModuleExportsInternal>((resolve, reject) => {
          require.async("../dotnet.runtime.js")
            .then(mod => resolve(mod))
            .catch(({ errMsg, mod }) => reject(errMsg));
        });
    }
    ...
    if (typeof jsModuleNativeAsset.moduleExports === "object") {
        ...
        //Rick jsModuleNativePromise = import(/*! webpackIgnore: true */jsModuleNativeAsset.resolvedUrl!);
        jsModuleNativePromise = new Promise<NativeModuleExportsInternal>((resolve, reject) => {
          require.async("../dotnet.native.js")
            .then(mod => resolve(mod))
            .catch(({ errMsg, mod }) => reject(errMsg));
        });
    }
