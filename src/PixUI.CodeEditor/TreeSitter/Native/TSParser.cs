...
namespace CodeEditor
{
    public class TSParser : IDisposable
    {
        ...
        public TSLanguage Language
        {
            ...
            get => new TSLanguage(ts_parser_language(_handle));
        }
        ...
        public TSTree Parse(string text, TSTree? oldTree = null, CancellationToken? token = null)
        {
            var ptr = Marshal.StringToHGlobalUni(text);
            try
            {
                return ParseInternal(ptr, (uint)text.Length * 2, InputEncoding.Utf16, oldTree,
                    token);
            }
            finally
            {
        ...
        /// <param name="token">cancellation token to use</param>
        /// <returns>the parsed syntax tree</returns>
        public unsafe TSTree Parse(byte[] bytes, InputEncoding encoding, TSTree? oldTree = null,
            CancellationToken? token = null)
        {
            fixed (byte* ptr = bytes)
            {
                return ParseInternal(new IntPtr(ptr), (uint)bytes.LongLength, encoding, oldTree,
                    token);
            }
        }
        ...
        private unsafe TSTree ParseInternal(IntPtr input, uint length, InputEncoding encoding,
            TSTree? oldTree, CancellationToken? token)
        {
            var cancelFlag = 0L;
        ...
        public LogCallback Logger
        {
            ...
            set
            {
                ...
                unsafe
                {
                    ...
                    {
                        log = (IntPtr)(delegate* unmanaged<IntPtr, TsLogType, IntPtr, void>)&InternalLogHandler,
                        payload = IntPtr.Zero,
                    });
                }
            }
        ...
        private static void InternalLogHandler(IntPtr payload, TsLogType logtype, IntPtr data)
        {
            var type = (LogType)logtype;
            var arg = Marshal.PtrToStringAnsi(data);
            //TODO: _logCallback(type, arg);
        }
        ...
        public void PrintDotGraphs(FileStream fs)
        {
            ...
            else
            {
                var handle = fs.SafeFileHandle ??
                             throw new ArgumentException("could not get file handle");
                var result = false;
        ...
        /// <see cref="Parse(string,CodeEditor)"/> or other parsing functions. If you don't want to resume,
