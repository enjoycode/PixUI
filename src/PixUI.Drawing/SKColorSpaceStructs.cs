...
public unsafe partial struct SKColorSpacePrimaries
{
    ...
    public SKColorSpacePrimaries(float rx, float ry, float gx, float gy, float bx, float by,
        float wx, float wy)
    {
        fRX = rx;
        fRY = ry;
...
public unsafe partial struct SKColorSpaceXyz
{
    ...
    [Obsolete("Use DisplayP3 instead.")] public static SKColorSpaceXyz Dcip3 => DisplayP3;
    ...
    public SKColorSpaceXyz(float[] values)
    {
        if (values == null)
            throw new ArgumentNullException(nameof(values));
        if (values.Length != 9)
            throw new ArgumentException("The matrix array must have a length of 9.",
                nameof(values));
        ...
        : this(SkiaApi.sk_colorspace_icc_profile_new(), true)
    {
        if (Handle == IntPtr.Zero)
            throw new InvalidOperationException(
                "Unable to create a new SKColorSpaceIccProfile instance.");
    }
